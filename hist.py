...
The design and functionality of the classes was inspired by ROOT project histogramming library.

The classes defined in this module do **NOT** provide drawing capabilities, so there is **NO** draw() nor paint()
method. Use other libraries like matplotlib to plot the data.

For convenience, another module is provided in this package for easy plotting using matplotlib. **See rootplots.mpl**
...
class HistND:
    ...
    def bins_to_cell(self, idxBins) -> int:
        """ returns the index of the cell that is located by the bin indexes of the axes
    ...
    def pos_to_cell(self, x) -> int:
        """ Returns the cell index (global linear bin) that contains the point at position 'x'.
         Where 'x' are the coordinates of a point in @self.dimension.
    ...
    def get_cells_contents_errors(self, includeEmptyBins=False) -> List[float]:
        ...
        Args:
            includeEmptyBins (bool): if empty cell are included in the list.
                If True then empty cell are included, otherwise they are NOT included
    ...
    def projection(self, keepDims):
        ...
        Args:
            keepDims (array-like): an array that contains the id's of the dimensions to keep when projecting.
    ...
    def fill_cell(self, i_cell: int, do_error_per_bin=True) -> int:
        ...
        Args:
            ...
            do_error_per_bin (bool): whether to compute weights per bins.
              Note: This is **expensive operation** because it computes
              the bins indexes (it decomposes cell index) and positions per axis before filling weights per bin.

        ** Its Recommended to use other fill_* methods**

        Returns:
            int. The **index of the affected cell (global linear bin) or -1** if no cell
    ...
    def fill_bins(self, b) -> int:
        ...
        Args:
            b (array-like): the bin indexes for each dimension.
            The size of the array is the same as the number of dimensions

            NOTE - b contains indexes of the bins on each axis, not the positions on axis
    ...
    def fill_pos_w(self, x, w: float) -> int:
        ...
        Args:
            x (array-like): the position for each dimension.
            The size of the array is the same as the number of dimensions
...
class Hist1D(HistND):
    ...
    def fill_w(self, x: float, w: float) -> int:
        ...
        see also::
            fill_cell_w(), fill_bins_w(), fill_pos_w()
        """
...
class Hist2D(HistND):
    ...
    def fill(self, x: float, y: float) -> int:
        ...
        Args:
            ...
            NOTE - x, y are coordinates not indexes of the bins
...
class Hist3D(HistND):
    ...
    def fill(self, x: float, y: float, z: float) -> int:
        ...
        Args:
            ...
            NOTE - x, y, z are coordinates not indexes of the bins
